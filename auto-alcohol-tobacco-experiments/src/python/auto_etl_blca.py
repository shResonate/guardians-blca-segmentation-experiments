...
def main():
    ...
    auto_data = read_data_from_s3(fs, args.auto_file_path, 251)
    auto_data = auto_data.drop_duplicates(subset='RID')
    newX = split_bottleneck(auto_data)
    pca = get_pca(newX, 12)
    pca_data = apply_pca_with_nulls(newX, pca)
    newX = pca_data.copy()
    newX.to_csv(args.s3_output_directory + 'auto_pca_data.csv', index=False)
    ...
    save_data_to_csv(recoded_data, args.s3_output_directory, 'auto_blca_input.csv')
    save_data_to_csv(recoded_data, args.local_directory, 'auto_blca_input.csv')
    inputFilePath = args.local_directory + 'auto_blca_input.csv'
    ...
    blcaRcodePath = '/Users/samhawala/Documents/GitHub/guardians-segmentation/blcaExperiments/autoExperiment/src/R/blca.R'
...
parser = argparse.ArgumentParser(description='Experiment auto Segmentation')

parser.add_argument('--useCase', type=str,
                    default="auto",
                    help='The current use-case for segmentation')

parser.add_argument('--auto_file_path', type=str,
                    default='s3://datasci-scantle/disruptCRM/bsm/auto/',
...
parser.add_argument('--local_directory', type=str,
                    default='/Users/samhawala/Documents/work2024/Segmentations/finalExperiments/autoExperiment/',
                    help='The location of the attribute hierarchy file')

parser.add_argument('--s3_output_directory', type=str,
                    default='s3://resonate-datasci-dev/shawala/Segmentations/autoExperiment/',
...
def recode_attributes(df, attributes):
    ...
    # recode auto attributes
    df['orderedautoOnline'] = np.where(df['319501'] == 319503, 1,
    ...
    df['likelyautoOnline'] = np.where(df['387406'] == 387407, 1,
